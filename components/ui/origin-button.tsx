...
function getCoverDiameter(width: number, height: number, x: number, y: number) {
  ...
  // Add safety padding to ensure 100% full coverage across border-radius and sub-pixel rounding
  return Math.ceil(2 * maxDistance) + 24;
}
...
const OriginButton = React.forwardRef<HTMLButtonElement, OriginButtonProps>(
  (
    ...
    useIsomorphicLayoutEffect(() => {
      ...
      const measure = () => {
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          setCoverSize(getCoverDiameter(rect.width, rect.height, ox, oy));
        }
      };
    ...
    return (
      ...
      >
        {/* Origin-fill ripple circle centered exactly at (origin.x, origin.y) */}
        <motion.span
          animate={{
