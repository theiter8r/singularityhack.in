...
const AccordionGallery = ({
  ...
  const handleClick = (i: number, e: MouseEvent<HTMLAnchorElement | HTMLDivElement>) => {
    if (i !== active) {
      e.preventDefault();
      setActive(i);
    }
  ...
  const handleKeyDown = (i: number, e: KeyboardEvent<HTMLAnchorElement | HTMLDivElement>) => {
      ...
      {items.map((item, i) => {
        ...
        return (
          <div
            ...
            onMouseEnter={() => handleEnter(i)}
            onFocus={() => setActive(i)}
            onKeyDown={(e) => handleKeyDown(i, e as unknown as KeyboardEvent<HTMLAnchorElement | HTMLDivElement>)}
            role="listitem"
            tabIndex={0}
            aria-current={isActive ? 'true' : undefined}
            aria-label={item.label}
